      ...
      try {
        ...
        const [pRes, msgRes] = await Promise.all([
          supabase.from("profiles")
            .select("id, full_name, username, avatar_url, city, province, bio")
        ...
        if (cancelled) return;
        setOther(pRes.data as OtherProfile);
  ...
  async function send(e: React.FormEvent) {
    ...
    try {
      const { data, error: iErr } = await supabase
        .from("messages")
        .insert({ match_id: matchId, sender_id: user.id, receiver_id: other.id, content })
        .select("id, sender_id, receiver_id, content, created_at")
        .single();
      if (iErr) throw iErr;
  ...
  if (error && !other) {
    return (
      <section className="mx-auto max-w-3xl px-4 py-24 text-center">
        ...
        <Link to="/messages" className="mt-6 inline-flex rounded-full border border-border px-6 py-3 text-sm font-semibold hover:bg-muted transition">
          Back to messages
        </Link>
      </section>
    );
    ...
    <section className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-6 md:py-10">
      <div className="rounded-3xl border border-border bg-card shadow-soft overflow-hidden flex flex-col" style={{ height: "calc(100vh - 8rem)" }}>
        ...
        <header className="flex items-center gap-3 border-b border-border p-4 bg-gradient-to-r from-pink/5 to-purple/5">
          <Link to="/messages" aria-label="Back" className="grid h-10 w-10 place-items-center rounded-full hover:bg-muted transition">
          ...
          <div className="min-w-0">
            <p className="font-display text-lg font-semibold truncate">
              {other?.full_name ?? other?.username ?? "Your match"}
            </p>
            {(other?.city || other?.province) && (
              <p className="text-xs text-muted-foreground inline-flex items-center gap-1">
                <MapPin className="h-3 w-3" /> {[other?.city, other?.province].filter(Boolean).join(", ")}
              </p>
            )}
          </div>
        </header>
        ...
        <div ref={scrollerRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-background/50">
          {messages.length === 0 && (
            <div className="mx-auto max-w-sm text-center py-10">
              <div className="mx-auto mb-3 grid h-14 w-14 place-items-center rounded-full bg-gradient-brand text-white shadow-glow">
        ...
        {error && (
          <div className="border-t border-destructive/30 bg-destructive/10 px-4 py-2 text-xs text-destructive">{error}</div>
        )}
        ...
        <form onSubmit={send} className="flex items-center gap-2 border-t border-border p-3 bg-card">
          <input
            ...
            className="flex-1 rounded-full bg-muted px-5 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-pink/40"
            maxLength={2000}
          />
          <button
            type="submit"
            ...
            aria-label="Send"
            className="grid h-12 w-12 place-items-center rounded-full bg-gradient-brand text-white shadow-soft hover:shadow-glow disabled:opacity-50 transition"
          >
